...
- [`rules/finish-the-task.md`](rules/finish-the-task.md) -- **the single work-discipline canon (5 pillars, all in one place):** finish to runtime · cover all related branches · quality over token economy · difficulty is no reason to defer · fix every exact reproduced cause immediately, without planning/review loops, using one minimal causal test before resuming runtime. Broad repeated reviews/tests are forbidden unless a concrete changed risk requires them. Enforced by `stop-phrase-guard` + `session-handoff-*`; near-overflow -> write a handoff.
